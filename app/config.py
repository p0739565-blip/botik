...
# ==========================
# Xray / device_guard (используется только на VM, не в боте/API)
# ==========================
...
XRAY_INBOUND_TAG = os.getenv("XRAY_INBOUND_TAG", "vless-reality-in")

# Окно, в течение которого IP считается "активным устройством".
# Если от IP не было новых подключений дольше этого времени, при следующем
# превышении лимита именно он освобождает место первым.
DEVICE_ACTIVE_WINDOW_MINUTES = _int_env("DEVICE_ACTIVE_WINDOW_MINUTES", 30)

# На сколько секунд банить IP через iptables при превышении лимита устройств
DEVICE_BAN_SECONDS = _int_env("DEVICE_BAN_SECONDS", 300)
