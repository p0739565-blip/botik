...
from app.vless_links import DEAD_LINKS, render_links
...
async def get_subscription(token: str, request: Request):
    ...
    if subscription is None or not subscription.is_valid:
        return build_subscription_response(0, None, DEAD_LINKS)

    links = render_links(subscription.user.vless_uuid)
